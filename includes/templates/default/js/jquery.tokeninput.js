...
*/
	...
	// My gives info and a link to show all users
	function show_dropdown_hint() {	
		
		// Clear out old results
		dropdown.empty();
				
		// Link for all users search
		$('<li>')
			.html("Show All Users in Project")
			.mouseover(function(event) {
				$(event.target).addClass(settings.classes.selectedDropdownItem);
			})
			.mouseout(function(event) {
				$(event.target).removeClass(settings.classes.selectedDropdownItem);
			})
			.mousedown(function(event) { 
				$.get(settings.url, {"q": "" }, function (results) {
		            populate_dropdown("", results);
		        }, "json");
			})
			.appendTo($('<ul class="'+settings.classes.dropdown+'">').appendTo(dropdown));
