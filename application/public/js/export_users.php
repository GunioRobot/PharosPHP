		...
		$('a#export-users').click(function(e) {
			...
			<?
				...
				for ( $i = 0; $i < $length; $i++ ) {
					...
					$sql = "SELECT DISTINCT MONTH(date_added) as month FROM users WHERE user_level = '".(int)Settings::get('application.users.levels.basic')."' AND YEAR(date_added) = '".$years[$i]."' ORDER BY month ASC";
